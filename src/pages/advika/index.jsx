import { mainAdvika, beritaTerbaru, beritaTrending } from '../../mocks/advikaMock';
		...
		<div className="max-w-xs w-full">
			...
			<div className="flex flex-col gap-3">
				{beritaTrending.map((item) => (
					<NavLink to={item.link} key={item.id} target="_blank" className="flex items-center gap-3 rounded-lg shadow p-2 hover:shadow-lg transition" style={{ minWidth: 0 }}>
						<img src={item.imagePath} alt={item.title} className="w-14 h-14 object-cover rounded" />
	...
	return (
		<div>
			{repeatedMock.map((item, index) => (
				<div key={`${item.id}-${index}`} className="flex flex-col lg:flex-row items-start gap-4 p-4 rounded-xl cursor-pointer shadow hover:shadow-lg hover:brightness-95 transition duration-300 mb-3">
					{/* Image Section */}
			...
			<div className="flex flex-col lg:flex-row justify-between mt-6 gap-x-1">
				<div>
					{mainAdvika
						...
						.map((item) => (
							<NavLink to={item.link} key={item.id} className="block" target="_blank">
								<div className="grid relative rounded-[.2rem] overflow-hidden w-full max-w-full hover:brightness-90 transition-all duration-500 ease-in-out mb-[4px] cursor-pointer">
									<img src={item.imagePath} alt={item.title} className="w-full h-auto object-cover row-start-1 col-start-1" />
									...
									<div className="row-start-1 col-start-1 self-end p-4 bg-gradient-to-t from-black/80 via-black/50 to-transparent text-white">
										...
										<p className="text-sm text-blue-300">
											{item.author} - <span className="text-white">{item.date}</span>
										</p>
									</div>
								</div>
							</NavLink>
					...
					{mainAdvika
						...
						.map((item) => (
							<div key={item.id} className="grid relative rounded-[.2rem] overflow-hidden w-full max-w-full hover:brightness-90 transition-all duration-500 ease-in-out mb-[1px] cursor-pointer">
								<NavLink to={item.link} target="_blank" className="w-full h-auto object-cover row-start-1 col-start-1 block">

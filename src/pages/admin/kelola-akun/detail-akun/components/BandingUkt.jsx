	...
	return (
		<div className="bg-white shadow-md rounded-md p-4 text-sm text-dark">
			...
			<div className="text-center font-medium text-sm">
				[{' '}
				<a href={`/admin/banding-ukt/${data.id}`} className="underline cursor-pointer hover:text-main-primary transition">
					Lihat Detail
				</a>{' '}
				]
			</div>
...
const BandingUkt = () => {
	...
	const filteredUktAppeals = useMemo(() => {
		return uktAppeals.filter((appeal) => appeal?.students?.id === student?.id);
	}, [uktAppeals, student?.id]);
	...
	return (
		<div className="space-y-6 p-4">
			...
			{/* Pagination */}
			{filteredUktAppeals?.length > 0 && <Pagination className="mt-8" currentPage={currentPage} totalPages={pagination.total_pages} onPageChange={handlePageChange} />}
		</div>
	);
};

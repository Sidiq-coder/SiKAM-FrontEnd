...
export default function DetailAdvika() {
  ...
  const handleSave = (id) => {
    setArticles(prev => 
      prev.map(article => 
        article.id === id ? { ...editedData } : article
      )
    );
    setEditingId(null);
    setEditedData({});
        ...
        {!(editingId === article.id) ? (
          <>
            <button
              ...
              className="flex items-center gap-2 px-3 py-1 bg-[#ED9E31] text-white text-xs rounded hover:opacity-80 transition-colors"
            >
          ...
          <>
            ...
            <button
              ...
              className="flex items-center gap-2 px-3 py-1 bg-[#DC3545] text-white text-xs rounded hover:opacity-80 transition-colors"
            >
...
const BackButton = () => (
  <NavLink 
    to="/admin/advika" 
    className="flex items-center gap-4 mb-8 pt-8 cursor-pointer hover:brightness-150 transition-all duration-300"
  >
...
const EditableArticleCard = ({ 
  ...
  return (
    <div className="border border-gray-200 rounded-lg p-6 bg-white shadow-sm hover:shadow-md transition-shadow">
      ...
      <div>
        {isEditing ? (
          <PDFUpload onPdfUpdate={handlePdfUpdate} />
        ) : (
          <PDFDownloadButton pdfPath={currentData.pdfPath || pdfPath} onPdfUpdate={handlePdfUpdate} />
        )}
      </div>
    </div>
  );

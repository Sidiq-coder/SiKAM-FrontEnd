...
import { faChevronLeft, faFile, faFilePdf, faFilePen } from "@fortawesome/free-solid-svg-icons";
...
export default function DetailAdvika() {
  ...
  return (
    <div className="px-10 md:px-20 lg:px-32 pb-1 min-h-screen mb-48">
      ...
      <BackButton />
      <Button label={`Edit Berita`} href={`/admin/advika/editAdvika/${params.id}`} icon={<FontAwesomeIcon icon={faFile}/>} className={`bg-[#ED9E31] text-white text-xs hover:opacity-80 transition-all transition-duration-500 cursor-pointer`}/>
      </div>
      {article ? (
        <>

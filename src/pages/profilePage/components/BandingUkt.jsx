  ...
  return (
    <div className="space-y-6 p-4">
      {pagedData.map((data, index) => (
        <div
          key={index}
          className="bg-white shadow-md rounded-md p-4 text-sm text-[#2A2A2A]"
        >
          <div className="mb-2 flex">
            <div className="w-40 font-semibold">ID pengajuan</div>
            <div>: <span className="text-[#A3A3A3]">{data.id}</span></div>
          </div>
          <div className="mb-2 flex">
            <div className="w-40 font-semibold">Tanggal pengajuan</div>
            <div>: <span className="text-[#A3A3A3]">{data.tanggal}</span></div>
          </div>
          <div className="mb-2 flex">
            <div className="w-40 font-semibold">Status</div>
            <div>
              : <span className={`${data.color} font-medium`}>{data.icon} {data.status}</span>
            </div>
          </div>
          <div className="mb-4 flex">
            <div className="w-40 font-semibold">Tipe bencana</div>
            <div>: <span className="text-[#A3A3A3]">{data.tipe}</span></div>
          </div>
